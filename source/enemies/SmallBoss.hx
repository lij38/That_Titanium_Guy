...
class SmallBoss extends Enemy {
	...
	private var damage_dist:Int = 200;
	...
	public function new(X:Float=0, Y:Float=0, bulletArray:FlxTypedGroup<Bullet>, gravity:Float) {
	...
	public function dash_attack(elapsed:Float):Void {
		...
		stuck_count++;
		if (stuck_count == 15) {
			bulletArray.add(new EnemyBullet(x, y + 120, 250, facing, 25, range));
		}
		if (stuck_count > 30) {
			stuck_count = 0;
	...
	public function attack(elapsed:Float):Void {
		...
		attack_count++;
		if (attack_count == 54) {
			bulletArray.add(new EnemyBullet(x, y + 120, 250, facing, 15, range));
		}
		if (attack_count > 67) {
			animation.finish();
			attack_count = 0;
