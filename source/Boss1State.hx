...
class Boss1State extends FlxState {
	...
	override public function create():Void {
		...
		_foreground.loadMapFromArray(cast(_map.getLayer("foreground"), TiledTileLayer).tileArray, _map.width, _map.height, 
			AssetPaths.boss1tile__png, _map.tileWidth, _map.tileHeight, FlxTilemapAutoTiling.OFF, 1051, 1, 1);

		_background = new FlxTilemap();
		_background.loadMapFromArray(cast(_map.getLayer("background"), TiledTileLayer).tileArray, _map.width, _map.height, 
			AssetPaths.boss1background__png, _map.tileWidth, _map.tileHeight, FlxTilemapAutoTiling.OFF, 1, 1, 1052);

		_foreground.follow();
		//_foreground.setTileProperties(2, FlxObject.NONE);
		//_foreground.setTileProperties(3, FlxObject.ANY);
		_background.follow();
		//_background.setTileProperties(2, FlxObject.NONE);
		//_background.setTileProperties(3, FlxObject.ANY);
		add(_background);
		add(_foreground);
